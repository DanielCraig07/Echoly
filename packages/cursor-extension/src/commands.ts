...
import { StatusBarHandle, StatusBarState, updateStatusBar } from './statusBar';
...
export async function probeCommand(statusBar: StatusBarHandle): Promise<void> {
  ...
  try {
    ...
    if (result.ok) {
      ...
      vscode.window.showInformationMessage(`Deepseek 连接成功 — 模型: ${modelLabel}`);
    } else {
      updateStatusBar(statusBar, {
      ...
      vscode.window.showErrorMessage(`Deepseek 连接失败: ${result.detail}`);
    }
  } catch (err) {
    updateStatusBar(statusBar, {
      state: StatusBarState.Disconnected,
...
export async function configureCommand(statusBar: StatusBarHandle): Promise<void> {
  ...
  try {
    ...
    if (result.success) {
      ...
      vscode.window.showInformationMessage(`配置成功! ${result.detail}\nCursor 现已指向内网模型。`);
    } else {
      updateStatusBar(statusBar, {
        state: StatusBarState.Disconnected,
        detail: result.detail,
    ...
    vscode.window.showErrorMessage(`配置异常: ${err instanceof Error ? err.message : String(err)}`);
  }
}
...
export async function startProxyCommand(statusBar: StatusBarHandle): Promise<void> {
  if (proxyManager?.isRunning()) {
    vscode.window.showWarningMessage(`代理服务器已在端口 ${proxyManager.port} 上运行`);
    return;
  }
